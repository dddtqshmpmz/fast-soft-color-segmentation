...
run_name = 'train_0621'
num_primary_color = 7
csv_path = 'ihc_30k.csv' # なんでも良い．後方でパスを置き換えるから
...
# 打印所有数据到日志
log = open("test_ihc_semantic_color_%s.log" % (run_name), "a")
sys.stdout = log
...
test_dataset = MyDatasetIHC(csv_path,num_primary_color, mode='test')
...
# try:
#     os.makedirs('results/%s/test_ihc_semantic_color' % (run_name))
# except OSError:
#     pass
...
with torch.no_grad():
    ...
    for batch_idx, (target_img, primary_color_layers,img_name) in enumerate(test_loader):
        print('img #', batch_idx)
        ...
        for i in range (len(RGBA_layers)):
            img_name_prefix = img_name[i].split('/')[-1]
            img_name_prefix = img_name_prefix.split('.')[0]
            for j in range (len(RGBA_layers[0])):
                # save_image(RGBA_layers[i,j, :, :, :], 'results/%s/test_ihc_semantic_color/%s_layer-%02d.png' % (run_name,img_name_prefix , j) )
                save_image(RGBA_layers[i,j, :, :, :],  os.path.join( '../../../../home','newDisk','diaoli', 'results', 'test_ihc_semantic_color', '%s_layer-%02d.png' % (img_name_prefix , j) ) )


        print('Saved to results/%s/test_ihc_semantic_color/...' % (run_name))
